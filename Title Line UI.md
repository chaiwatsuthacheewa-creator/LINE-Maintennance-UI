...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Loading Overlay */
  #loadingState { position: fixed; top:0; left:0; width:100%; height:100%; background: var(--bg-color); z-index: 2000; display: flex; flex-direction: column; justify-content: center; align-items: center; color: var(--primary); font-size: 1.2rem;}
...
<div class="header">
  <div class="header-left">
    <a href="URL_หน้าเมนูหลัก" class="back-btn">
...
<div class="dashboard-grid">
  ...
  <div class="card span-2">
    <div class="card-title" style="color: var(--danger);">⚠️ งานค้างเกิน 48 ชม. (SLA)</div>
...
<script>
  // ==========================================
  // 🔴 1. ใส่ URL ของ Web App Google Apps Script ตรงนี้
  // ==========================================
  const API_URL = "ใส่_WEB_APP_URL_ที่ได้จากขั้นตอนที่_1_ตรงนี้"; 

  // --- ฟังก์ชันแปลง ชั่วโมง เป็น วัน/ชั่วโมง ---
  ...
  // --- ฟังก์ชันแปลงวันที่จาก Sheet (DD/MM/YYYY HH:mm:ss) ให้เป็น Date Object ของ JS ---
  function parseCustomDate(dateStr) {
    ...
    try {
      ...
      // JS Date: Year, Month(0-11), Day, Hour, Min, Sec
  ...
  function updateClock() {
    ...
    dateEl.innerText = `${days[now.getDay()]} ${now.getDate().toString().padStart(2,'0')}/${(now.getMonth()+1).toString().padStart(2,'0')}/${now.getFullYear().toString().slice(-2)}`;
  }
  ...
  let buildingMap = {}; // ตัวแปรเก็บข้อมูลตึก
  let mainData = []; // เก็บ Data ของจริงที่ดึงมา
  let myChart = null; // เก็บตัวแปรกราฟ

  // ==========================================
  // 🔵 2. ฟังก์ชันโหลดข้อมูลและเรนเดอร์หน้าจอ
  // ==========================================
  async function loadData() {
    try {
      ...
      rawData.forEach(row => {
        // ประมวลผลแต่ละแถวที่ดึงมาจาก Sheet
        const start = parseCustomDate(row.Date);
        const end = row.CloseTime ? parseCustomDate(row.CloseTime) : null;
        const status = row.Status === 'Complete' ? 'completed' : 'pending';
        
        // เช็ค Building Code ให้ตรง (เช่นจาก Sheet เป็น "ari" ก็จับคู่กับคีย์)
        let bCode = (row.Building || '').toLowerCase().trim();
        if(!buildingMap[bCode]) bCode = 'ari'; // เผื่อพิมพ์ผิด ให้ลง ari ไว้ก่อน

        // คำนวณชั่วโมงใหม่จาก Date และ CloseTime (หรือเวลาปัจจุบันถ้ายังไม่เสร็จ)
        ...
        mainData.push({
          ...
          status: status,
          isOverSLA: durationInHours > 48 && status === 'pending'
        });
      ...
      renderDashboard(); // เรียกฟังก์ชันวาด UI
      document.getElementById('loadingState').style.display = 'none'; // ปิดหน้าจอโหลด
      
    } catch (error) {
      console.error('Error loading data:', error);
      document.getElementById('loadingState').innerHTML = `<div style="color:var(--danger)">เกิดข้อผิดพลาดในการโหลดข้อมูล <br><small>โปรดตรวจสอบ API_URL</small></div>`;
    }
  }
  ...
  function renderDashboard() {
    ...
    // --- Render Chart (Dynamic ตามข้อมูลจริง) ---
    renderChart();
  }

  function renderChart() {
    // คำนวณข้อมูลกราฟตามวันที่
    const daysMap = {};
    mainData.forEach(d => {
      if(!d.start) return;
      const day = d.start.getDate(); // ดึงเฉพาะวันที่
    ...
    if (myChart) myChart.destroy(); // เคลียร์กราฟเก่าถ้ามี
  ...
  function openModal(buildingKey) {
    ...
    if(bData.length === 0) {
      ...
      bData.forEach(job => {
        ...
        const dateStr = job.start ? `${job.start.getDate()}/${job.start.getMonth()+1}/${job.start.getFullYear().toString().slice(-2)}` : '-';
        jobsHTML += `<div class="job-card"><div class="job-card-header"><span style="font-weight: 500; color: var(--text-main);">${job.ticket} - ${job.detail}</span>${badge}</div><div style="display: flex; justify-content: space-between; color: var(--text-muted);"><span>📅 ${dateStr}</span><span>⏱ ${formatDuration(job.duration)}</span></div></div>`;
      });
    }
